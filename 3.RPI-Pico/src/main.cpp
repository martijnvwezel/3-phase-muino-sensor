...
int main() {
    ...
    while (true) {
        ...
        set_pin_io(LED, true);
        sleep_ms(15);
        uint32_t sen_a = read_sensor(0);
        uint32_t sen_b = read_sensor(1);
        uint32_t sen_c = read_sensor(2);
        ...
        uint32_t sen_a_zero = read_sensor(0);
        uint32_t sen_b_zero = read_sensor(1);
        uint32_t sen_c_zero = read_sensor(2);
