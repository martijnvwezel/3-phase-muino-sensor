...
public:
    ...
    void loop(int32_t sen_a, int32_t sen_b, int32_t sen_c, int32_t sen_a_dark, int32_t sen_b_dark, int32_t sen_c_dark) {

        this->sen_a = sen_a;
        this->sen_b = sen_b;
        this->sen_c = sen_c;

        int32_t sen_a_zero = sen_a_zero;
        int32_t sen_b_zero = sen_b_zero;
        int32_t sen_c_zero = sen_c_zero;
    ...
    int phase_fine_iter(int a, int b, int c) {
        ...
        for (int i = -AC_STEPS; i <= (AC_STEPS * 2); i++) {
            ...
            float cora          = a * __builtin_cos(x);
            float corb          = b * __builtin_cos(x + (PI2_3));
            float corc          = c * __builtin_cos(x - (PI2_3));
